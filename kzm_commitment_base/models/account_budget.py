...
class BudgetLine(models.Model):
    ...
    def _get_sql_query(self):
        return """
            ...
            WHERE account_id=%s
                AND (CAST(date AS DATE) between to_date(CAST(%s AS CHAR),'YYYY-MM-DD')
                AND to_date(CAST(%s AS CHAR),'YYYY-MM-DD'))
        """
...
class BudgetPositionCommitmentLimit(models.Model):
    ...
    def check_amount_limit_inferior_global_limit(self):
        for rec in self:
            if rec.user_id.commitment_global_limit and \
                    rec.amount_limit > rec.user_id.commitment_global_limit:
