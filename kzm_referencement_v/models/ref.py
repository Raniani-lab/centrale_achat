...
class Ref(models.Model):
    ...
    ref_val_date = fields.Date("Validation date",default=fields.Date.today())
    ref_asked_by = fields.Many2one('res.users', string="Requested by", default=lambda self: self.env.uid)
    ref_responsible = fields.Many2one('res.users', string="Responsible", default=lambda self: self.env.uid)
